<div class="home mt-5 mr-8 bg-[#F4F6FA] h-auto rounded px-10">
    ...
    <div class="justify-between grid grid-cols-2 items-center px-7 mx-auto">
        <h1 class="text-[20px] font-semibold text-sky-600 my-5">List Products</h1>

        <a href="index.php?act=Add_Product" class=" bg-yellow-300 text-center  py-2 px-2 rounded-md hover:bg-[#BAAACE] hover:text-white mx-4 font-semibold text-[#551AA9] my-5">ADD
            Products</a>
    </div>
    ...
    <table class="table shadow-2xl shadow-cyan-500/50 mx-auto border-2 border-cyan-500/50mx-auto my-5">
        ...
        foreach ($ListProduct as $ListPro) {
            ...
            $Update_Product = "index.php?act=Update_Product&id=" . $id;
            $Delete_product = "index.php?act=Delete_product&id=" . $id;
            $image2 = "../upload/" . $image2;
            $cate = Get_Category_By_Code($id_categories);
    ...
    ?>
                ...
                <td class="border-2 border-cyan-500/50 relative py-4 px-2 text-center">
                    <a class="py-1 absolute top-[15px] left-1 px-4 bg-[#1E74A4] hover:bg-[#BAAACE] hover:text-white text-white rounded-md mx-2" href="<?= $Update_Product ?>">Update</a>
                    <a onclick="return confirm('Bạn có muốn xóa không')" class="text-white absolute left-1 hover:bg-[#BAAACE] hover:text-white bg-[#AC3131] py-1 px-4 rounded-md mx-2" href="<?= $Delete_product ?>">Deltete</a>
                </td>
            </tr>
